...
class SocialGraph:
    ...
    def get_all_social_paths(self, user_id):
        ...
        path = [user_id]  # {1: {8, 10, 5}, 2: {10, 5, 7}, 3: {4}, 4: {9, 3}, 5: {8, 1, 2}, 6: {10}, 7: {2}, 8: {1, 5}, 9: {4}, 10: {1, 2, 6}}
        # visited = {1: [1]}
        q = Queue()    
        q.enqueue(path)  # q = [],   [  , [1,10,2,7], [1,10,2,10]]

        while q.size() > 0:
            path = q.dequeue()   # [1]  [1,8] [1,10]  [1,10,6 ]  [1,10,2]
            user = path[-1]  # 1 ---     8     10           6       2

            if user not in visited:
                visited[user] = path    # visited = {1: [1], 8: [1,8], 10: [1,10], 5: [1, 5], 6: [1,10,6], 2: {1,10,2} 7 {1,10,2, 7} }
                for friend in self.friendships[user]:
                    
                    new_path = list(path)  #[1,10,2,5]]
